...
public class PrimaryPrincipalAuthenticationStrategy extends AllSuccessfulStrategy {

	private static final Logger log = LoggerFactory.getLogger(PrimaryPrincipalAuthenticationStrategy.class);

	@Override
	public AuthenticationInfo afterAttempt(Realm realm, AuthenticationToken token, AuthenticationInfo info,
			AuthenticationInfo aggregate, Throwable t) {
	...
	private void validatePrimaryPrincipals(AuthenticationInfo info, AuthenticationInfo aggregate, Realm realm) {
		...
		if (!aggregPrincipal.equals(infoPrincipal)) {
			String message = "All realms are required to return the same primary principal. Offending realm: "
					+ realm.getName();
			log.debug(message);
			throw new AuthenticationException(message);
		}
